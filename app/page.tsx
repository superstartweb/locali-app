...
import { 
  ...
  Smile, GlassWater, Home as HomeIcon, Trash2, Beef, Fish, Globe
} from 'lucide-react'
...
export default function Home() {
  ...
  // 1. Chiediamo la posizione dell'utente solo per il filtro, non per il salvataggio automatico
  useEffect(() => {
  ...
  const fetchLocali = async () => {
    ...
    if (searchTerm) {
      ...
      // Quelli "Lontani" o "Senza GPS"
      const extra = (tutti || []).filter((t: any) => !idsVicini.has(t.id))
      
      finalData = [...(vicini || []), ...extra]
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Se lat/lng sono nulli, passiamo 0 per la funzione SQL
    const { error } = await supabase.rpc('save_locale', {
      ...
      n_lat: newLocale.lat || 0, n_lng: newLocale.lng || 0, n_visitato: isVisitato
    })
        ...
        <div className="max-w-xl mx-auto">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">La tua mappa del gusto</p>
            </div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-50 p-4 pl-12 rounded-2xl font-bold border-2 border-transparent focus:border-amber-400 outline-none transition-all text-sm"
              value={searchTerm}
          ...
          {!searchTerm && (
            <div className="bg-slate-50 p-4 rounded-[2rem] border border-slate-100">
               <div className="flex justify-between items-center mb-2 px-1">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Raggio vicini</span>
      ...
      <div className="max-w-xl mx-auto p-5 space-y-6">
        {locali.map((l) => (
          <div key={l.id} onClick={() => {setNewLocale({...l, note: l.note || '', lat: l.posizione?.coordinates?.[1] || null, lng: l.posizione?.coordinates?.[0] || null}); setIsModalOpen(true)}} className={`bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden transition-all active:scale-[0.98] ${l.rating_cibo === 1 ? 'opacity-60' : ''}`}>
            ...
            {/* DISTANZA O FALLBACK */}
            <div className={`absolute top-6 left-6 px-3 py-1 rounded-full text-[9px] font-black uppercase shadow-md ${l.distanza_km && l.distanza_km < 50 ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}>
               {l.distanza_km && l.distanza_km < 50 ? (l.distanza_km < 1 ? `${Math.round(l.distanza_km * 1000)} m` : `${l.distanza_km.toFixed(1)} km`) : <div className="flex items-center gap-1"><Globe size={10}/> Instagram / Web</div>}
            </div>
            ...
            <div className="mt-8 mb-6">
              ...
              {l.visitato ? (
                ...
                <div className="bg-amber-50/50 p-4 rounded-3xl border border-amber-100 flex justify-between items-center px-6">
                   ...
                   <span className="text-[9px] font-bold text-amber-500 underline uppercase">Vota</span>
                </div>
              )}
            </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-md flex flex-col justify-end">
          ...
          <div className="bg-white w-full rounded-t-[3rem] p-8 pb-20 shadow-2xl max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-full duration-500 relative">
            <h2 className="text-2xl font-black italic text-slate-900 mb-8 border-b border-slate-50 pb-4">{newLocale.id ? 'Aggiorna Stella' : 'Nuova Stella ✨'}</h2>
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => { setNewLocale({...newLocale, lat: userPos?.lat || 0, lng: userPos?.lng || 0}); alert("Posizione Attuale Salvata! 📡") }} className={`absolute right-2 top-2 p-3 rounded-xl shadow-sm border transition-all ${newLocale.lat ? 'bg-amber-400 text-white' : 'bg-white text-amber-500'}`}> <MapPin size={24}/> </button>
              ...
              <div className="space-y-4 pt-4">
                 <button type="submit" className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black text-xs uppercase shadow-2xl active:scale-95"> <Check size={20}/> SALVA STELLA </button>
